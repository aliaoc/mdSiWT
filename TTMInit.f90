...
!          T_melt=933.25D0, T_abl=2740.0D0                ! Not used here
        ...
        IF(KEYBS.GE.5) OPTPEN = 100.0d0  ! the following parameters are for Si
...
!       Defining preliminary step of discretization
        adifx = DABS(xdiff)*1.0D-10/DBLE(nlcx)*2.0d0               ! [m]
        adify = DABS(ydiff)*1.0D-10/DBLE(nlcy)*2.0d0               ! [m]
        adifz = DABS(zdiff)*1.0D-10/DBLE(nlcz)*2.0d0               ! [m]
...
!          Xsh, Ysh, Zsh in MPI; XED, YED, ZED in TTM equation
        ...
        IF(TIME.NE.0.0d0) dift = DELTA*1.0d-12/10.0d0
...
!       4 link cell layers is usually enough for bulk margines.
        marbulk = 2 ! Used for bulk simulations
...
!                   The following relates to the case of substrate 
...
!        alfas   = 1.0d0/(OPTPEN+ballis)
        ...
        SourceL = ENRIN*SQRT(4.0d0*Log(2.0d0)/PI)/(DURAT*1.0d-12)/(XL*YL*1.0d-20)

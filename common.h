...
!     Parameters for_ 3-body term of SW and NASA-SW
      COMMON/SW/ SW_sig(KPMX),SW_lam(KPMX),SW_gam(KPMX),SW_eps(KPMX)
      COMMON/SWN/ SW_TH0(KPMX),SW_alph(KPMX),FDSW(3,LPMX),STENSW(LPMX,3,3)

!     Parameters for TF and TFM	
      COMMON/TF/ DFA(KTMX,NT),DFC(KTMX,NT)
      COMMON/TF2/ FA(KTMX,NT),FC(KTMX,NT),FFR(KTMX,NT),DFR(KTMX,NT)
      COMMON/TFN0/ TF_Bet(KPMX)
      COMMON/TFN2/ TF_C(KPMX),TF_D(KPMX),TF_H(KPMX)
      COMMON/TFN3/ DFCDFA(KTMX,NT),DFCDFR(KTMX,NT)
      COMMON/TFN4/ TF_N(KPMX),TF_KSI(KPMX),FDTF(3,LPMX),STENTF(LPMX,3,3)
      COMMON/TFMOD/ TF_C1,TF_C2,TF_C3,TF_C4,TF_C5,TF_a,TF_alpha,TF_beta,delSF
...
!     Order parameter for_ identification of liquid
      COMMON/LIQ/ CUTLIQ,CUTLIQ1,CUTLIQ2,CUTLIQ3,alat,ExpCoef
...
!     Nordsieck integrator
      ...
      COMMON/NNN/ NN1,NNF,NN13,NCIRCr,NCIRCl,NCIRC,NCIRC2,NPOTS,ISTEP,NLREN,NOTEM
      ...
      COMMON/DEPID/ x_dep,y_dep,IDDEP
...
!     otherwise chose another place for_ variables adding 
      ...
      PARAMETER (NNONREFL2 = 2)
      COMMON/FNN2/ FSTART2,GIMPED2

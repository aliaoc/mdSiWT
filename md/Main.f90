...
!        Pair potentials and forces are tabulated
         IF (KEYBS.EQ.4) Then
            CALL EF1_EAM(1,2,2)  ! Cu(1),Au(2),Ni(3),Fe(4),Zr(5),Al(6)
            IF(NTYPE.GE.2) THEN
               CALL EF1_EAM(2,8,8)
               CALL EF1_EAM(3,2,8)
            ENDIF
         ...
         ELSEIF(KEYBS.EQ.5) THEN
            CALL EF1_SW(1,1,1) ! Si(1) SW potential, Avinash
            IF(NTYPE.GE.2) THEN
               CALL EF1_EAM(2,8,8)
               CALL EF1LJ(3,18,20)
            ENDIF
         ...
         ELSEIF (KEYBS.EQ.7) Then
            CALL EF1_TF(1,1,1)  ! Si TF potential
            IF(NTYPE.GE.2) THEN
               CALL EF1_EAM(2,8,8)
               CALL EF1LJ(3,18,20)
            ENDIF
         ...
         ELSEIF(KEYBS.EQ.8) Then
            CALL EF1_TFM(1,1,1)  ! Si TFM potential
            IF(NTYPE.GE.2) THEN
               CALL EF1_EAM(2,8,8)
               CALL EF1LJ(3,18,20)
            ENDIF
...
!            ZCRIT = ZCRIT + delta
            ...
            CALL COLLECT()
            CALL MPI_BCAST(POTTG,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
               IF(POTTG0.LT.POTTG) THEN
                  exit Loop_outer
                  CALL MPI_FINALIZE(info)
                  STOP
               ELSE
                  IF(mypid.eq.0) THEN
                     WRITE(300,300) J,POTTG,J*delta,XD(3,1)-zmarker
                     print *, J, J*delta, XD(3,1) - zmarker
300                  FORMAT(I6,1x,E20.10,1x,E20.10,1x,E20.10)
                     POTTG0 = POTTG
                  ENDIF
               ENDIF
            ENDDO Loop_outer
...
!        Before starting laser time should be set to zero
         ...
         IDDEP = 0
         IF(mypid.eq.0) OPEN(UNIT = 433,FILE = 'displacement.out')
...
!           The following is used for simulation of the deposition            
            IF(ISTEP.EQ.40.AND.IDDEP.EQ.0.AND.NDEP.EQ.3) CALL DEPOSITION()
            IF(MOD(ISTEP,1).EQ.0.AND.NDEP.EQ.3) THEN
               DO I=1,NN1
                  IF(KHIST(I).EQ.9) THEN
                     rmsd = SQRT((XD(1,I)-x_dep)*(XD(1,I)-x_dep) + &
                          (XD(2,I)-y_dep)*(XD(2,I)-y_dep))
                     WRITE(433,*) time,rmsd
                  ENDIF
               ENDDO
            ENDIF
            ...
            IF(KFLAG.EQ.5) THEN
               ...
               CALL BERE_T(TEMPTR,TEMPTR1,TEMPTR2)
            ENDIF

            IF(KEYDEP.EQ.2.AND.LGOT.EQ.2) CALL HEATING_NP()
...
!           Writing data for future analysis
            IF(MOD(ISTEP,NWRITE).EQ.0.OR.ISTEP.EQ.NSTEP.OR.ISTEP.EQ.1) Then
               ...
               IF (LGOT.EQ.2) THEN
                  IF((MOD(ISTEP,8000).EQ.0.OR.ISTEP.EQ.NSTEP).AND.NSTEP.NE.1) THEN
                     CALL SwriteTTM_Res()
                     NFSTEP = ISTEP
                  ELSE

...
!     To use it,need to have alat defined in Eftab for example
      SUBROUTINE Symmetry(NCS,NO_TYPE)
        INCLUDE 'common.h'
        INCLUDE 'mpif.h'
!       Choose 12 neighbores for CSP construction
        ...
        CALL Temper(QINT,POTT,TEMPTR,TEMPTR1,TEMPTR2)
        IF(NO_TYPE.EQ.2) THEN
           CALL MPI_REDUCE(TEMPTR1,TEMPTR1G,1,MPI_DOUBLE_PRECISION,MPI_SUM,0, &
                MPI_COMM_WORLD,ierr)
           CALL MPI_BCAST(TEMPTR1G,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
        ELSE
           CALL MPI_REDUCE(TEMPTR2,TEMPTR2G,1,MPI_DOUBLE_PRECISION,MPI_SUM,0, &
                MPI_COMM_WORLD,ierr)
           CALL MPI_BCAST(TEMPTR2G,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
        ENDIF
           
        IF(NO_TYPE.EQ.2) THEN
           ExpCoef = 4.293d-06 ! for Si
        ELSE
           ExpCoef = 1.922962811d-05 ! For a typical metal
        ENDIF

        IF(ALLPBC) THEN
           alatT = alat
        ELSE
           alatT = alat*(1.0d0+ExpCoef*(TEMPTR1G-273.0d0))  ! approximate lattice parameter
        ENDIF
        
!       Defining a cutoff distance so that the first two neighbor shells
!       in a perfect fcc lattice are included
        IF(NO_TYPE.EQ.2) THEN
           RADCUT = 0.6d0*alatT ! 0.57005974153d0*alatT
        ELSE
           RADCUT = 0.775d0*alatT
        ENDIF
...
!       of each atom using the neighbor list
        outer: DO I=1,NN1
           ...
           IF(KTYPE(I).EQ.NO_TYPE) CYCLE outer
           ...
           inner: DO JJ=1,NNG(I)  ! full list is assumed here
              ...
              IF(KTYPE(J).EQ.NO_TYPE) CYCLE inner
...
!       Constructing CSP out of MMG neighbores
        If(NO_TYPE.EQ.2) THEN
           ...
           i_loop:DO I = 1,NN1
              ...
              IF(KTYPE(I).EQ.NO_TYPE) CYCLE i_loop
              ...
              j_loop:DO k = 1,NLIM
                 ...
                 IF(KTYPE(J).EQ.NO_TYPE) CYCLE j_loop
                 ...
                 FD(1,I) = FD(1,I) + XCSJ
                 FD(2,I) = FD(2,I) + YCSJ  ! collecting data for numenator
                 FD(3,I) = FD(3,I) + ZCSJ
                 
                 denominator = denominator + XCSJ*XCSJ + YCSJ*YCSJ + ZCSJ*ZCSJ
                 
              ENDDO j_loop
              
              FD(2,i) = (FD(1,I)**2+FD(2,I)**2+FD(3,I)**2) / (4.d0*denominator)            ! CSP
!           if (fd(2,i).lt.0.d0) print *, "csp:", i, j, fd(2,i), nlim,ktype(i), ktype(j),mypid
!           if (fd(2,i).gt.1.d0)  print *, "csp:", i, j, fd(2,i), nlim, ktype(i),ktype(j),mypid
        ...
        ELSE
           ...
           ii_loop:DO I = 1,NN1
              ...
              IF(KTYPE(I).EQ.NO_TYPE) CYCLE ii_loop
              ...
              ENDIF
              KKG(1:NLIM) = .FALSE.
              jj_loop:DO k = 1,NLIM-1
                 IF(KKG(k)) CYCLE jj_loop
                 J = MMMG(k,I)

                 IF(KTYPE(J).EQ.NO_TYPE) CYCLE jj_loop
                 ...
                 jjj_loop:DO m = k + 1,NLIM
                    IF(KKG(m)) CYCLE jjj_loop
                    JJ = MMMG(m,I)
                    
                    IF(KTYPE(JJ).EQ.NO_TYPE) CYCLE jjj_loop
...
!       Averaging over neighboring atoms
        o_loop: DO I=1,NN1
           IF(KTYPE(I).EQ.NO_TYPE) CYCLE o_loop
           FD(3,I) = FD(2,I)
           in_loop: DO KK=1,MMG(I)
              J=MMMG(KK,I)
              IF(KTYPE(J).EQ.NO_TYPE) CYCLE in_loop

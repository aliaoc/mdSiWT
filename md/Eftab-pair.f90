...
!             Element(KE2),' potential' 
        ...
        ENDIF
           ...
           LJCeps = 0.35d0/32.0d0 !LJeps(KE1)
           LJCsig = 2.4d0 !LJsig(KE1)
           RM(KTEF) = rcc
        
!       Uncomment this line if want to plot the potential
        IF(mypid.eq.0)OPEN (UNIT = 111,FILE='LJ-'//Element(KE1)//'-'//Element(KE2)//'.pot')
        sig6 = LJCsig**6
        DX=RM(KTEF)/NT
        XT=0.0d0
        DO I=1,NT
...
!          Uncomment this line if want to plot the potential
           IF(mypid.eq.0)WRITE (111,*) XT,UT(KTEF,I),FT(KTEF,I)
!          Transfer to program units
           UT(KTEF,I) = UT(KTEF,I)/ENUNIT!!*0.0d0
           FT(KTEF,I) = FT(KTEF,I)/ENUNIT!*0.0d0
        END DO
!W      Uncomment this line if want to plot the potential
        IF(mypid.eq.0) CLOSE (UNIT = 111)
        RETURN

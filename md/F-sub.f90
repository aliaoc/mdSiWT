...
!     at some position ZCRIT. Will work with Z components only.
      SUBROUTINE F_SUB()
        ...
        PARAMETER (cigma =  2.57321d0)    !Au-Au at 300 K
        PARAMETER (epsil = 0.4415d0/ENUNIT/10.0d0)   !Au-Au

!        YSTR  = DABS(DBLE(NDEP) - YCENTR)
...
!                       IF(NDEP.NE.0) THEN
!                          YLEN = DABS(XD(2,I)-YCENTR)
!                          IF(YLEN.GE.YSTR) xfrag = 1.0d0 + &
!                               9.0d0*(YLEN-YSTR)/(0.5d0*YLREAL-YSTR)
!                       ENDIF

                       FSUB = xfrag*48.0d0*epsil*DZ6*(DZ6-0.5d0)/DZ
                       USUB = xfrag*4.0d0*epsil*DZ6*(DZ6-1.0d0)

...
10                  CONTINUE
                    smsgbuf(l)     = FDTF(1,i)
                    smsgbuf(l + 1) = FDTF(2,i)
                    smsgbuf(l + 2) = FDTF(3,i)
                    l = l + 3
...
!                   so we will not pass STENSW until we do need it
                    IF(MOD(ISTEP,NEPRT).EQ.0.OR.ISTEP.EQ.1) THEN
                       smsgbuf(l)     = STENTF(i,1,1)
                       smsgbuf(l + 1) = STENTF(i,2,2)
                       smsgbuf(l + 2) = STENTF(i,3,3)
                       l = l + 3
                    ENDIF
...
!             current link cell util it is completed
              FDTF(1,idx) = FDTF(1,idx) + rmsgbuf(l)
              FDTF(2,idx) = FDTF(2,idx) + rmsgbuf(l + 1)
              FDTF(3,idx) = FDTF(3,idx) + rmsgbuf(l + 2)
              l = l + 3

              IF(MOD(ISTEP,NEPRT).EQ.0.OR.ISTEP.EQ.1) THEN
                 STENTF(idx,1,1) = STENTF(idx,1,1) + rmsgbuf(l)
                 STENTF(idx,2,2) = STENTF(idx,2,2) + rmsgbuf(l + 1)
                 STENTF(idx,3,3) = STENTF(idx,3,3) + rmsgbuf(l + 2)
                 l= l + 3
              ENDIF

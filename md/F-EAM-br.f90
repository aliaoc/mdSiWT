!     Calculation of Forces and Energies with Johnson's EAM potential
      SUBROUTINE F_EAM_br()
        ...
        DIMENSION FVZ(12),FVZG(12),FVZA(12)
        DIMENSION NSEQ(6*nxnodes*nynodes)
...
!       To calculate forces need also information about density in the skin
        ...
        outer_loop: DO I=1,NN1
           ...
           IF(KTYPE(I).NE.KWT) CYCLE outer_loop
...
!           IF(KHIST(I).GT.10) RI = (XD(3,I) - ZCENTR)

           inner_loop: DO K=1,NNG(I)
              J=NNNG(k,i)

              IF(KTYPE(J).NE.KWT) CYCLE inner_loop
...
!                 RJ = XD(3,J) - ZCENTR
!                 DJI = DABS(RJ) - DABS(RI)
                 IF(KTYPE(J).EQ.1) THEN
                    DJI = XD(3,J) - XD(3,I)
                 ELSEIF(KTYPE(J).EQ.2) THEN
                    DJI = XD(3,I) - XD(3,J)
                 ELSE
                    print *,"unexpected case in F-EAM-br.f90. The program will halt"
                    STOP
                 ENDIF
...
!          Potential from EAM Embedding Function (with Rho as input)
           ...
           IF(KHIST(I).GT.10) THEN
              ...
              FVZ(KK+6) = FVZ(KK+6) + Q1D(3,I)/DELTA
              NRL = NRL + 1
              LNONREF(NRL) = I
           ENDIF
        ...
        IF(mypid.NE.0.AND.INON.EQ.1) CALL MPI_SEND(FVZ, &
             12,MPI_DOUBLE_PRECISION,0,0,MPI_COMM_WORLD,ierror)

        IF(mypid.eq.0)  THEN
           FVZG(1:12) = 0.0d0
           FVZG(1:12) = FVZG(1:12) + FVZ(1:12)*INON
           DO nrec = 1,INONG-INON
              CALL MPI_RECV(FVZA,12,MPI_DOUBLE_PRECISION, &
                   MPI_ANY_SOURCE,0,MPI_COMM_WORLD,status1,ierror)
              NSEQ(nrec) = status1(MPI_SOURCE)
              FVZG(1:12) = FVZG(1:12) + FVZA(1:12)
           ENDDO

           IF(NCIRC.NE.0) THEN
              FVZG(1:3) = FVZG(1:3)/DBLE(NCIRC)
              FVZG(7:9) = FVZG(7:9)/DBLE(NCIRC)
           ELSE
              FVZG(1:3) = 0.0d0
              FVZG(7:9) = 0.0d0
           ENDIF
           ...
           IF(NCIRC2.NE.0) THEN
              FVZG(4:6) = FVZG(4:6)/DBLE(NCIRC2)
              FVZG(10:12) = FVZG(10:12)/DBLE(NCIRC2)
           ELSE
              FVZG(4:6) = 0.0d0
              FVZG(10:12) = 0.0d0
           ENDIF
           ...
           DO nrec = 1,INONG-INON
              CALL MPI_SEND(FVZG,12,MPI_DOUBLE_PRECISION, &
        ...
        IF(mypid.NE.0.AND.INON.EQ.1) CALL MPI_RECV(FVZG, &
             12,MPI_DOUBLE_PRECISION,0,0,MPI_COMM_WORLD,status2,ierror)

        
        IF(mypid.eq.0) THEN
           WRITE(411,412) FVZG(1) - FSTART - GIMPED*FVZG(7), &
                FVZG(4) - FSTART2 - GIMPED2*FVZG(10),FVZG(1),FVZG(7),ISTEP
...
!       Adding/substracting forces due to the NRB
        DO II=1,NRL
           ...
           IF(KK.GE.1.AND.KK.LE.3)THEN
              IF(NCIRC.NE.0) THEN
                 FD(3,I) = FVZG(KK) - FSTART - GIMPED*FVZG(KK+6)
                 EOUT = EOUT + GIMPED*FVZG(KK+6)*FVZG(KK+6)*ENUNIT*DELTA/3.0d0
              ENDIF   
           ELSE
              IF(NCIRC2.NE.0) THEN
                 FD(3,I) = FVZG(KK) - FSTART2 - GIMPED2*FVZG(KK+6)
                 EOUT = EOUT + GIMPED2*FVZG(KK+6)*FVZG(KK+6)*ENUNIT*DELTA/3.0d0
              ENDIF
           ENDIF

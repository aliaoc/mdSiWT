...
!     Leonid Zhigilei and Dmitri Ivanov, 2002
      SUBROUTINE SwriteTTM()
        INCLUDE 'common.h'
        INCLUDE 'commonTTM.h'
        INCLUDE 'mpif.h'
        REAL*8, DIMENSION(:), ALLOCATABLE :: xmsgbuf
        REAL*8, DIMENSION(:), ALLOCATABLE :: XEDT,YEDT,ZEDT
        REAL*8, DIMENSION(:), ALLOCATABLE :: T_lG,T_eG,TPSXG,RONG,FSOLG,Q1VG
        INTEGER statusttm(MPI_STATUS_SIZE)
        CHARACTER*5 chtime, chpid

        IF(mypid.eq.0) CALL CPU_TIME(timestart0)
        
        Call Symmetry(4,2) ! NO_TYPE  = 2, type of exclusion, NSC = 4
        Call Symmetry(12,1) ! Calculation of the central symmetry parameter
...
!       And later we set FLUID flag back to false        
...
!       into the output file
        IF(mypid.eq.0) THEN
           ...
           ALLOCATE(XEDT(nlcg),YEDT(nlcg),ZEDT(nlcg),T_lG(nlcg), &
                T_eG(nlcg),TPSXG(nlcg),RONG(nlcg),FSOLG(nlcg),Q1VG(nlcg))
...
!                   CALL CPU_TIME(time00)
!                   print *,time00-timestart0," SwriteTTM received from",ifromttm
!                   call Flushout(6)
           ...
           DO iz = 1,nlczg
              DO iy = 1,nlcyg
                 DO ix = 1,nlcxg
                    ...
                    IF(KONF.EQ.1) THEN
                       IF(iy.eq.nlcyg/2) WRITE(989,515) YEDT(ip),XEDT(ip),ZEDT(ip), &
...
!           CLOSE(UNIT = 979)
           ...
           DEALLOCATE(XEDT,YEDT,ZEDT,T_lG,T_eG,TPSXG,RONG,FSOLG,Q1VG)

           CALL CPU_TIME(time00)
           print *," "
           print *,"TTM part of the output data is written",time00-timestart0
           print *," "
!           call Flushout(6)

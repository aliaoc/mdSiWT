...
!     Berendsen et al., J. Chem. Phys., 81 (1984), p. 3684.
      SUBROUTINE BERE_P (TPRESS)
        INCLUDE 'common.h'
        INCLUDE 'mpif.h'
        REAL*8, PARAMETER :: pbeta=0.005d0
...
!     Leonid Zhigilei, 2002
      SUBROUTINE BERE_T(TEMPTR,TEMPTR1,TEMPTR2)
        INCLUDE 'common.h'
        INCLUDE 'mpif.h'

        IF(KEYBS.EQ.5.AND.NTYPE.EQ.2) THEN
...
!     the thermal bath.  It is usually choosen from the range of 0.1 - 2 ps.
        TAU=5.0d0
        IF(KEYBS.EQ.5.AND.NTYPE.EQ.2) THEN
           Vscale=SQRT(1.0d0+(TEMPTRG/TEMPTR1G-1.0d0)*DELTA/TAU)
        ELSE
        ...
        DO I=1,NN1
           IF(KEYBS.EQ.5.AND.NTYPE.EQ.2) THEN
              IF(KTYPE(I).EQ.1) THEN ! Scale for Si only
